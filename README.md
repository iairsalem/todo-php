...
Based on php-login-one-file a simple approach to authentication. This could be useful to small vanilla-php projects that require registration, login.

Most important change: it adds a is_admin column (that must be set separately) so even if everyone can register, only some have admin privileges.

Sqlite-powered, could also work with mysql due to PDO.
